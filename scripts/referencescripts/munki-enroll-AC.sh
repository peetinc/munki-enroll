...
SUBMITURL="https://munki.artichoke.consulting/ac/enroll/enroll.php"
# Change this to a local admin account you have if you have one
ADMINACCOUNT="artichoke"
#######################

# Make sure there is an active Internet connection
SHORTURL=$(/bin/echo "$SUBMITURL" | /usr/bin/awk -F/ '{print $3}')
PINGTEST=$(/sbin/ping -o -t 4 google.com | /usr/bin/grep "64 bytes")
#PINGTEST=$(/sbin/ping -o -t 4 "$SHORTURL" | /usr/bin/grep "timeout")
