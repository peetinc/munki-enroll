...
// require cfpropertylist and logger
...
// User-set variables
// Provide default manifest and catalog
$defaultmanifest = 'Management/Mandatory';
$defaultcatalog = 'production';
////
// Munki Repo Varibles
// Manifest and catalogs relative path (include trailing / i.e. '../manifests/')
$manifestspath = '../manifests/';
$catalogspath = '../catalogs/';
//// End User Variables

// Variables provided by munki-enroll.sh
//if $function is not passed, set to enroll
if (!($function = filter_input(INPUT_GET, 'function', FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES)))
	{
		$function = 'enroll'; 
	}
...
//// Functions
function minVarCheck($recordname, $displayname, $uuid, $catalog1, $catalog2, $catalog3, $manifest1, $manifest2, $manifest3, $manifest4) {
// end if not enough variables provided
...
// Ensure provided catalogs and manifests exist.
function varCheck($recordname, $displayname, $uuid, $catalog1, $catalog2, $catalog3, $manifest1, $manifest2, $manifest3, $manifest4) {
if ( !file_exists( $catalogspath . $catalog1 ) )
	{
	echo "$catalog1 does not exist... \n";
	$exit = '999';
	}
if ( !file_exists( $catalogspath . $catalog2 ) )
	{
	echo "$catalog1 does not exist... \n";
	$exit = '999';
	}
if ( !file_exists( $catalogspath . $catalog3 ) )
	{
	echo "$catalog1 does not exist... \n";
	$exit = '999';
	}
if ( !file_exists( $manifestspath . $manifest1 ) )
	{
	echo "$manifest1 does not exist... \n";
	$exit = '999';
	}
if ( !file_exists( $manifestspath . $manifest2 ) )
	{
	echo "$manifest2 does not exist... \n";
	$exit = '999';
	}
if ( !file_exists( $manifestspath . $manifest3 ) )
	{
	echo "$manifest3 does not exist... \n";
	$exit = '999';
	}
if ( !file_exists( $manifestspath . $manifest4 ) )
	{
	echo "$manifest4 does not exist... \n";
	$exit = '999';
	}
if ( $exit == 999 )
	{
	...
	echo "Provided catalog(s) and manifest(s) exist. So there's that... \n\n";
	}
}

// Needs to be fixed for $function=update or $function=enroll
...
if ( file_exists( $manifestspath . $recordname ) && $function == 'enroll' )
	{
